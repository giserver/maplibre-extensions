...
if(!import.meta.env?.VITE_MAPBOX_TOKEN){
    const error = "需要在docs文件夹下创建一个.env.development文件, 并提供mapbox access token 记作 VITE_MAPBOX_TOKEN ";
    alert(error);
...
function handleMapOptionsInit(options: Omit<MapOptions, "container">){
    ...
    options.transformRequest = Mapbox.createRequestTransform(import.meta.env.VITE_MAPBOX_TOKEN);
}
