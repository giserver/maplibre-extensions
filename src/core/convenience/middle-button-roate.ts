...
type HandlerResult = {
    ...
    cameraAnimation?: (map: maplibregl.Map) => any;
...
namespace DOM {
    export function mouseButton(e: MouseEvent): number {
        if (e.type === "mousedown" || e.type === "mouseup") {
            if (typeof (window as any).InstallTrigger !== "undefined" && e.button === 2 && e.ctrlKey && window.navigator.platform.toUpperCase().indexOf("MAC") >= 0) {
...
export class MiddleButtonRoate {
    ...
    constructor(
        ...
        if (options.withPitch) this.mousePitch = new MousePitchHandler({ clickTolerance: ((map as any).dragRotate as any)._mousePitch._clickTolerance });
